...
const columns = [
  ...
  },
  {
    Header: 'Total',
    accessor: (order) => {
      return <div>{formatRupiah(sumPrice(order.order_items) + order.delivery_fee)}</div>;
    },
  },
  {
    Header: 'Invoice',
...
export default function UserOrders() {
  ...
  return (
    <LayoutOne>
      ...
      <br />
      <Table items={pesanan} totalItems={count} columns={columns} onPageChange={(page) => setPage(page)} page={page} isLoading={status === 'process'} />
    </LayoutOne>
  );
}
