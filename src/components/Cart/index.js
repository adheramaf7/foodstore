...
export default function Cart({ items, onItemInc, onItemDec, onCheckout }) {
  ...
  return (
    <div>
      <div className="text-3xl flex items-center text-red-700 pl-5">
        <FaCartPlus />
        <div className="ml-3">Keranjang</div>
      </div>
      ...
      <div className="px-2 border-b mt-5 pb-5">
        <Button text="Checkout" fitContainer iconAfter={<FaArrowRight />} disabled={!items.length} onClick={onCheckout} />
      </div>
      {!items.length ? (
        <div
        ...
        {items.map((item, index) => {
          return (
            <div key={index} className="mb-2">
              <CardItem imgUrl={`${config.api_host}/upload/${item.image_url}`} name={item.name} qty={item.qty} color="orange" onInc={(_) => onItemInc(item)} onDec={(_) => onItemDec(item)} />
            </div>
          );
        })}
      </div>
